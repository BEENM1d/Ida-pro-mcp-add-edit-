...
def list_local_types():
    ...
    try:
        # 尝试通过 idati 对象获取
        if hasattr(idati, 'get_ordinal_limit'):
            type_count = idati.get_ordinal_limit()
        else:
            # 使用固定范围扫描
            type_count = 1000
    except:
    ...
    for ordinal in range(1, min(type_count, 1000)):  # 限制最大数量避免无限循环
        try:
            tif = ida_typeinf.tinfo_t()
            if tif.get_numbered_type(idati, ordinal):
                ...
                locals.append(f"Type #{ordinal}: {type_name}")
                
                # 尝试获取类型声明
                try:
                    if tif.is_udt():
                        c_decl_flags = (ida_typeinf.PRTYPE_MULTI | ida_typeinf.PRTYPE_TYPE | ida_typeinf.PRTYPE_SEMI)
                        c_decl_output = tif._print(None, c_decl_flags)
                        if c_decl_output:
                            locals.append(f"  Declaration: {c_decl_output}")
                except:
                    pass
        except:
            continue
    
    return locals if locals else ["No local types found"]
...
@idawrite
def create_function(
    address: Annotated[str, "Address to create function at"],
) -> str:
...
@idawrite
def delete_function(
    address: Annotated[str, "Address of function to delete"],
) -> str:
    ...
    if ida_funcs.del_func(func.start_ea):
        return f"Successfully deleted function at {hex(ea)}"
    else:
        raise IDAError(f"Failed to delete function at {hex(ea)}")
...
@idawrite
def create_struct(
    struct_name: Annotated[str, "Name of the structure to create"],
    members: Annotated[str, "Structure members in C format"],
) -> str:
    """Create a new structure definition"""
    c_declaration = f"struct {struct_name} {{ {members} }};"
    
    flags = ida_typeinf.PT_SIL | ida_typeinf.PT_TYP
    errors, messages = parse_decls_ctypes(c_declaration, flags)
    
    if errors > 0:
        raise IDAError(f"Failed to create struct {struct_name}: {messages}")
    
    return f"Successfully created struct {struct_name}"

@jsonrpc
@idawrite
def add_struct_member(
    struct_name: Annotated[str, "Name of the existing structure"],
    member_name: Annotated[str, "Name of the new member"],
    member_type: Annotated[str, "Type of the new member"],
    offset: Annotated[int, "Offset of the member in bytes"],
) -> str:
    """Add a member to an existing structure"""
    # Get the structure type
    tif = ida_typeinf.tinfo_t()
    if not tif.get_named_type(None, struct_name, ida_typeinf.BTF_STRUCT):
        raise IDAError(f"Structure {struct_name} not found")
    
    # Get UDT details
    udt = ida_typeinf.udt_type_data_t()
    if not tif.get_udt_details(udt):
        raise IDAError(f"Failed to get structure details for {struct_name}")
    
    # Create new member
    member_tif = get_type_by_name(member_type)
    udm = ida_typeinf.udm_t()
    udm.name = member_name
    udm.type = member_tif
    udm.offset = offset * 8  # Convert to bits
    ...
    if not new_tif.create_udt(udt, ida_typeinf.BTF_STRUCT):
        raise IDAError(f"Failed to update structure {struct_name}")
    
    if not new_tif.set_named_type(None, struct_name, ida_typeinf.NTF_REPLACE):
        raise IDAError(f"Failed to save updated structure {struct_name}")
    
    return f"Successfully added member {member_name} to struct {struct_name}"

@jsonrpc
@idawrite
def create_enum(
    enum_name: Annotated[str, "Name of the enumeration to create"],
    members: Annotated[str, "Enum members in format 'NAME1=value1,NAME2=value2'"],
) -> str:
    """Create a new enumeration"""
    c_declaration = f"enum {enum_name} {{ {members} }};"
    
    flags = ida_typeinf.PT_SIL | ida_typeinf.PT_TYP
    errors, messages = parse_decls_ctypes(c_declaration, flags)
    
    if errors > 0:
        raise IDAError(f"Failed to create enum {enum_name}: {messages}")
    
    return f"Successfully created enum {enum_name}"

@jsonrpc
@idawrite
def set_data_type(
    address: Annotated[str, "Address to set data type at"],
    data_type: Annotated[str, "Data type to set"],
    size: Annotated[int, "Size of the data in bytes"],
) -> str:
...
def create_array(
    ...
    element_type: Annotated[str, "Type of array elements"],
    count: Annotated[int, "Number of elements in the array"],
) -> str:
    """Create an array at the specified address"""
    ea = parse_address(address)
    
    # Get element type
    elem_tif = get_type_by_name(element_type)
    
    # Create array type
    array_tif = ida_typeinf.tinfo_t()
    if not array_tif.create_array(elem_tif, count):
        raise IDAError(f"Failed to create array type")
    
    # Apply the array type
    if ida_typeinf.apply_tinfo(ea, array_tif, ida_typeinf.TINFO_DEFINITE):
        return f"Successfully created array of {count} {element_type} elements at {hex(ea)}"
    else:
...
def add_cross_reference(
    from_address: Annotated[str, "Source address of the cross reference"],
    to_address: Annotated[str, "Target address of the cross reference"],
    ref_type: Annotated[str, "Type of reference (code, data)"],
) -> str:
    """Add a cross reference between two addresses"""
    from_ea = parse_address(from_address)
    to_ea = parse_address(to_address)
    
    # Map reference type
    if ref_type.lower() == "code":
        xref_type = ida_xref.fl_CN  # Code near call
    elif ref_type.lower() == "data":
        xref_type = ida_xref.dr_R   # Data read
    else:
        raise IDAError(f"Unknown reference type: {ref_type}")
    
    # Add the cross reference
    ida_xref.add_cref(from_ea, to_ea, xref_type)
    return f"Successfully added {ref_type} cross reference from {hex(from_ea)} to {hex(to_ea)}"
...
def delete_cross_reference(
    ...
    ida_xref.del_cref(from_ea, to_ea, True)
    return f"Successfully deleted cross reference from {hex(from_ea)} to {hex(to_ea)}"
