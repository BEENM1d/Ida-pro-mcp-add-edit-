...
def patch_bytes(address: str, hex_bytes: str) -> str:
    ...
    try:
        result = make_jsonrpc_request("patch_bytes", address, hex_bytes)
        return result
    except Exception as e:
        return f"Failed to patch bytes: {str(e)}"

@mcp.tool()
def create_function(address: str) -> str:
    """Create a new function at the specified address"""
    try:
        result = make_jsonrpc_request("create_function", address)
        return result
    except Exception as e:
...
def delete_function(address: str) -> str:
    ...
    try:
        result = make_jsonrpc_request("delete_function", address)
        return result
    except Exception as e:
        return f"Failed to delete function: {str(e)}"

@mcp.tool()
def set_instruction_operand_type(address: str, operand_index: int, operand_type: str) -> str:
    """Set the operand type for an instruction"""
    try:
        result = make_jsonrpc_request("set_instruction_operand_type", address, operand_index, operand_type)
        return result
    except Exception as e:
        return f"Failed to set operand type: {str(e)}"

@mcp.tool()
def create_struct(struct_name: str, members: str) -> str:
    """Create a new structure definition"""
    try:
        result = make_jsonrpc_request("create_struct", struct_name, members)
        return result
    except Exception as e:
        return f"Failed to create struct: {str(e)}"

@mcp.tool()
def add_struct_member(struct_name: str, member_name: str, member_type: str, offset: int) -> str:
    """Add a member to an existing structure"""
    try:
        result = make_jsonrpc_request("add_struct_member", struct_name, member_name, member_type, offset)
        return result
    except Exception as e:
        return f"Failed to add struct member: {str(e)}"

@mcp.tool()
def create_enum(enum_name: str, members: str) -> str:
    """Create a new enumeration"""
    try:
        result = make_jsonrpc_request("create_enum", enum_name, members)
        return result
    except Exception as e:
        return f"Failed to create enum: {str(e)}"

@mcp.tool()
def set_data_type(address: str, data_type: str, size: int) -> str:
    """Set the data type at a specific address"""
    try:
        result = make_jsonrpc_request("set_data_type", address, data_type, size)
        return result
    except Exception as e:
        return f"Failed to set data type: {str(e)}"

@mcp.tool()
def create_array(address: str, element_type: str, count: int) -> str:
    """Create an array at the specified address"""
    try:
        result = make_jsonrpc_request("create_array", address, element_type, count)
        return result
    except Exception as e:
        return f"Failed to create array: {str(e)}"

@mcp.tool()
def add_cross_reference(from_address: str, to_address: str, ref_type: str) -> str:
    """Add a cross reference between two addresses"""
    try:
        result = make_jsonrpc_request("add_cross_reference", from_address, to_address, ref_type)
        return result
    except Exception as e:
        return f"Failed to add cross reference: {str(e)}"

@mcp.tool()
def delete_cross_reference(from_address: str, to_address: str) -> str:
    """Delete a cross reference between two addresses"""
    try:
        result = make_jsonrpc_request("delete_cross_reference", from_address, to_address)
        return result
    except Exception as e:
        return f"Failed to delete cross reference: {str(e)}"
